...
class SentimentIntensityAnalyzer(object):
    ...
    def make_lex_dict(self, lex_dict, lex_sep):
        """
        Convert lexicon file to a dictionary
        """
        lex = {}
        with open(lex_dict,'r') as f:
            for line in f.readlines():
                (word, measure) = line.strip().split(lex_sep)[0:2]
                lex[word] = float(measure)
        return lex
    ...
    def make_emoji_dict(self, emoji_dict, emoji_sep):
        ...
        with open(emoji_dict,'r') as f:
            for line in f.readlines():
                (line, description) = line.strip().split(emoji_sep)[0:2]
                emoji[line] = description
        return emoji
